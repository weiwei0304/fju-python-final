...
if st.session_state.step == 1:
    with st.container():
        st.markdown('<div class="step-wrapper-header">把自己還給自己</div>', unsafe_allow_html=True)
    ...
    with st.container():
        ...
        with col_center:
            has_quit_date = st.radio("是否有確定離職日期?", options=["是", "否"],
            index=0 if st.session_state.has_quit_date == "是" else 1 if st.session_state.has_quit_date == "否" else None
            )
        ...
        if has_quit_date == "否":
            ...
            if st.session_state.start_date > date.today():
                st.info(f"您的到職日 {st.session_state.start_date.strftime('%Y-%m-%d')} 尚未到來，到職後即可提出離職（預告期為 0 天）")
            else:
                st.success(f"最早可以提離職的時間是: {effective_date.strftime('%Y-%m-%d')}")
